    ...
    if (data.cssStyle.chatboxLocation) {
      ...
      if (utils.isFromMobile() && data.cssStyle.chatButtonMobileLocation) {
          if (data.cssStyle.chatButtonMobileLocation == 'circle-left' || data.cssStyle.chatButtonMobileLocation == 'circle-right') {
              ...
              btn.getElementsByTagName('img')[0].style.margin = '0'
              btn.getElementsByTagName('img')[0].style.padding = '0'
              btn.getElementsByTagName('img')[0].style.transform = 'unset'
              btn.getElementsByTagName('img')[0].style.transition = 'unset'
...
var utm_source = ''   //    广告投放平台
var utm_medium = ''   //    广告投放渠道
var utm_term = ''   //    推广关键字
var utm_content = ''  //    推广内容
var utm_campaign = ''   //    对应关键词
...
var getSeoParams = function () {
  ...
  if (href.indexOf('?') > 0) {
    var query_arr = href.split('?')[1].split('&')
    query_arr.forEach(function (query) {
      var temp = query.split('=')
      if (temp[0] == 'utm_source') {
        utm_source = temp[1]
      }
      if (temp[0] == 'utm_medium') {
        utm_medium = temp[1]
      }
      if (temp[0] == 'utm_term') {
        utm_term = temp[1]
      }
      if (temp[0] == 'utm_content') {
        utm_content = temp[1]
      }
      if (temp[0] == 'utm_campaign') {
        utm_campaign = temp[1]
      }
    })
  }
...
function QiMoIMSDK () {
  ...
  this.initChat = function (data, callback) {
    ...
    if (utils.getBroswer() != utils.broswer.IE6 && utils.getBroswer() != utils.broswer.IE7) {
      document.getElementById('qimo_chatpup').getElementsByTagName('iframe')[0].src =
        qimo_config.chatPage +
        '?v=' + qimo_config.version +
        '&clientId=' + qimo_config.clientId +
        '&urlTitle=' + qimo_config.urlTitle +
        '&fromUrl=' + qimo_config.fromUrl +
        '&serviceStates=' + data.serviceStates +
        '&accessId=' + data.accessId +
        '&styleColor=' + data.cssStyle.styleColor +
        '&peers=' + encodeURIComponent(JSON.stringify(data.peers)) +
        '&otherParams=' + qimo_config.otherParams +
        '&seoSource=' + encodeURIComponent(seoSource) +
        '&seoKeywords=' + encodeURIComponent(seoKeywords) +
        '&companyName=' + encodeURIComponent(qimo_config.companyName) +
        '&seoUrl=' + encodeURIComponent(lastReferrer) +
        '&language=' + qimo_config.language +
        '&href=' + href
    }
    this.loadStyle()
    var btn = document.getElementById('chatBtn')
  ...
  this.initInviteBox = function () {
    ...
    qimo_invite_close_btn && utils.eventOn(qimo_invite_close_btn, 'click', function () {
      ...
      if (onlineData.closeStayTime === '0') {
        utils.setCookie('close_stay', '1', 'session')
      }
    })
    // 默认按钮事件
  ...
  this.openInviteBox = function () {
    if (utils.chatboxProp.isChatBoxShow) {
      return
    }
  ...
  this.loadStyle = function () {
    ...
    if (utils.isFromMobile()) {
      style.href = qimo_config.chatHost + '/stylesheets/chatStyle_wap.css?0620'
      // 这里通过js赋值body为fixed 且高宽100%，是为了避免移动端窗口展开后依然能滚动父页面的问题。
      var body = document.getElementsByTagName('body')
      body[0].setAttribute('data-7m', '7m_warp')
    } else {
      style.href = qimo_config.chatHost + '/stylesheets/chatStyle.css?0620'
    }
  ...
  var openChatTodo = function () {
    ...
    if (utils.getBroswer() != utils.broswer.IE6 && utils.getBroswer() != utils.broswer.IE7) {
      ...
      if (chatBox.getElementsByTagName('iframe')[0].contentWindow.postMessage) {
        ...
        var chatWin = window.open(qimo_config.chatPage +
          '?v=' + qimo_config.version +
          '&clientId=' + (qimo_config.clientId || '') +
          '&urlTitle=' + qimo_config.urlTitle +
          '&fromUrl=' + qimo_config.fromUrl +
          '&serviceStates=' + utils.chatboxProp.isServiceTime +
          '&newWin=true&accessId=' + qimo_config.accessId +
          '&styleColor=' + styleColor +
          '&peers=' + encodeURIComponent(JSON.stringify(peers)) +
          '&peer=' + selectPeer +
          '&otherParams=' + qimo_config.otherParams +
          '&seoSource=' + encodeURIComponent(seoSource) +
          '&seoKeywords=' + encodeURIComponent(seoKeywords) +
          ...
          '&companyName=' + encodeURIComponent(qimo_config.companyName),
          'chatWin',
          'width=380px,height=535px,toolbar=no, menubar=no, scrollbars=no, resizable=yes, location=no, status=no')
        chatWin.focus()
      }
      chatBtn.style.display = 'none'
...
(function (doc) {
  ...
  function getMsgFromChildFrame (b) {
    ...
    if (b.data == 'loadSocketJs') {
      ...
      var btn = document.getElementById('chatBtn')
      //  此处需判断autoShow的值是否为true，若不为true，则不能展示开始会话按钮
      if (qimo_config.autoShow && qimo_config.autoShow !== 'false') {
        btn.style.display = 'block'
      }
